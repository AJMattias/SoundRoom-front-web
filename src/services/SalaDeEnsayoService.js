...
import { STORAGE_USER, getJwtToken, getMyRooms, setMyRooms } from '../storage/LocalStorage' 
...
class SalaDeEnsayoService {
    ...
    async uploadMultipleImages(formData, config) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('Error de respuesta de Fetch:', response.status, errorData);
            ...
            if (!(error instanceof ApiException)) {
                throw new ApiException(0, 'NETWORK_ERROR', 'Error de red o inesperado al subir imágenes.');
            }
    ...
    async update( room){
        ...
        if(roomCreated){
            ...
            const roomIndex = rooms.findIndex((r) => r.id === roomCreated.sala.id)
            if(roomIndex !== -1){
                rooms[roomIndex] = roomCreated
                setMyRooms(rooms)
            }else{
                console.log('room not found in local storage')
            }
        }
        return roomCreated
    }

    async saveRoom2(nameSalaDeEnsayo, calleDireccion, descripcion, precioHora, comodidades, tipoSala){
